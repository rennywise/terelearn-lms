...
if (strtolower((string)($submission['status'] ?? '')) === 'graded') {
    json_out(['status' => 'error', 'message' => 'Graded submissions cannot be changed.'], 409);
}
...
$updateStmt->bind_param('s', $submission['id']);
$ok = $updateStmt->execute();
$updateStmt->close();

if (!$ok) {
    json_out(['status' => 'error', 'message' => 'Failed to remove file.'], 500);
}

json_out(['status' => 'success', 'message' => 'Submitted file removed.']);
