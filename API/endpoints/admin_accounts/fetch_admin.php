...
if (!empty($user_id)) {
    // Return JSON response for single admin - fetch from both tables
    ...
    $sql = "SELECT 
            ...
            a.is_superadmin
            FROM tbluser u
            LEFT JOIN tbladmin a ON u.email = a.email AND a.is_deleted = 0
            WHERE u.id = ? AND u.user_level_id IN (1, 4) AND u.is_deleted = 0";
...
if (!empty($search)) {
    ...
    $sql = "SELECT u.id, u.email, u.username, u.user_level_id, u.is_active, a.first_name, a.last_name
            FROM tbluser u
            LEFT JOIN tbladmin a ON u.email = a.email AND a.is_deleted = 0
            WHERE u.user_level_id IN (1, 4) AND u.is_deleted = 0 
            AND (u.email LIKE ? OR u.username LIKE ? OR a.first_name LIKE ? OR a.last_name LIKE ?)
            ORDER BY u.user_level_id ASC, u.id ASC";
